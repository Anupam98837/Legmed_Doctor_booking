...
class PagePrivilegeController extends Controller
{
    ...
    /**
     * Normalize API path for privilege mapping.
     * Examples:
     *  - "api/users" -> "/users"
    ...
    protected function normalizeApiPath(string $path): string
    {
        ...
        $normalized = preg_replace('#^/api(?:/|$)#i', '/', $normalized) ?: $normalized;
    ...
    /**
     * Normalize assigned API list and return JSON or null.
     */
    protected function encodeAssignedApisOrNull($value): ?string
    {
        if ($value === null) {
            return null;
        }
        ...
        if (!is_array($value)) {
            return $this->encodeJsonOrNull($value);
        }
        ...
        foreach ($value as $item) {
            ...
            $normalized[] = $this->normalizeApiPath($item);
        }
                ...
                DB::transaction(function () use (
                    ...
                    foreach ($request->input('privileges', []) as $idx => $row) {
                        ...
                        if ($hasAssigned && array_key_exists('assigned_apis', $row)) {
                            $payload['assigned_apis'] = $this->encodeAssignedApisOrNull($row['assigned_apis']);
                        }
        ...
        try {
            ...
            $id = DB::transaction(function () use (
                ...
                if ($hasAssigned && $request->has('assigned_apis')) {
                    $payload['assigned_apis'] = $this->encodeAssignedApisOrNull($request->input('assigned_apis'));
                }
    ...
    public function update(Request $request, $identifier)
    {
        ...
        if (Schema::hasColumn('page_privilege', 'assigned_apis') && $request->has('assigned_apis')) {
            $update['assigned_apis'] = $this->encodeAssignedApisOrNull($request->input('assigned_apis'));
        }
            ...
            DB::transaction(function () use ($request, &$updated, &$skipped, &$errors, &$seenKeys) {
                ...
                foreach ($rows as $idx => $row) {
                    ...
                    if (Schema::hasColumn('page_privilege', 'assigned_apis') && array_key_exists('assigned_apis', $row)) {
                        $update['assigned_apis'] = $this->encodeAssignedApisOrNull($row['assigned_apis']);
                    }
    ...
    public function indexOfApi(Request $request)
    {
        ...
        foreach ($routes as $route) {
            ...
            // Keep only API controllers.
            if (!Str::startsWith($controllerFqcn, 'App\\Http\\Controllers\\API\\')) continue;

            $base = class_basename($controllerFqcn);
            $controllerKey = lcfirst($base);
            $path = $this->normalizeApiPath((string) $route->uri());
            ...
            foreach ($methods as $m) {
                $bucket = strtolower($m);
                if (!isset($out['Api'][$controllerKey][$path])) {
