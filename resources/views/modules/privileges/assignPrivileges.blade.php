...
.ap-head-copy .seg-muted{
  color:var(--muted-color);
  font-weight:500;
}

.ap-head-copy .seg-sep{
  color:var(--muted-color);
  opacity:.7;
}
...
/* Profile mini (professional card) */
.ap-profile{
  display:flex;
  gap:12px;
  align-items:center;
...
.ap-search-wrap .form-control{
  padding-left:34px;
  background:var(--surface);
  border-color:var(--line-strong);
}

.ap-search-wrap .form-control:focus{
  border-color:var(--primary-color);
  box-shadow:0 0 0 2px rgba(var(--primary-rgb), .16);
}
...
.ap-dd .dropdown-item i{
  width:16px;
  text-align:center;
  opacity:.85;
}

.ap-dd .dropdown-item:hover{
  background:var(--surface-2);
}

.ap-dd .dropdown-item.text-danger{
  color:var(--danger-color) !important;
}

.ap-dd .dropdown-item.text-danger:hover{
...
.ap-accordion .accordion-item:last-child{
  border-bottom:0;
}

.ap-accordion .accordion-header{
  position:relative;
  z-index:1;
}
...
html.theme-dark .ap-placeholder{
...
html.theme-dark .ap-profile{
  background:#0f172a;
  border-color:var(--line-strong);
}

html.theme-dark .ap-card,
html.theme-dark .ap-card-head{
  background:#0f172a;
}

html.theme-dark .ap-priv-table thead th{
  background:#0f172a;
  border-color:var(--line-strong);
  color:#94a3b8;
}

html.theme-dark .ap-priv-table tbody tr{
  border-color:var(--line-soft);
}

html.theme-dark .ap-module-pill{
  background:#1e293b;
}

html.theme-dark .ap-module-tools{
  background:#1e293b;
}

html.theme-dark .ap-tools{
  background:#1e293b;
}

html.theme-dark .ap-chip{
  background:#0f172a;
}
...
<div class="ap-wrap">
  ...
  {{-- Header Panel (matching other pages) --}}
  <div class="panel ap-head">
    <div class="ap-head-row">
      <div class="ap-head-copy">
        <h1>
          <span class="seg-muted">Users</span>
      ...
      <div class="ap-head-actions">
        <button type="button" class="w3-icon-btn ap-refresh-btn" id="btnRefresh" title="Refresh page">
  ...
  <div class="ap-card">
    ...
    <div class="ap-card-body">
      ...
      <div id="modulesContainer" class="accordion ap-accordion">
        ...
        <div class="p-3">
          <div class="ap-placeholder mb-2"></div>
          <div class="ap-placeholder mb-2"></div>
          <div class="ap-placeholder mb-2"></div>
  ...
  <div class="ap-small-muted mt-3 d-flex align-items-center gap-2">
    <i class="fa fa-circle-info"></i>
    <span>Tip: Use <b>Select all</b> at the top for every privilege, or the per-module <b>Select all</b> to control one module. Changes are only saved after you click <strong>Save</strong>.</span>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', ()=> {
  // [The JavaScript code remains exactly the same as your original]
  // I'm keeping your original JavaScript intact since it's functional
  // and only the UI needed to be matched
  ...
  function showGlobalLoading(show){
    if (globalLoading) globalLoading.style.display = show ? 'flex' : 'none';
  }
  ...
  function setButtonLoading(button, loading){
    ...
    if (loading){
      ...
      button.innerHTML = '<i class="fa fa-save me-1"></i>Save';
    }
  }
  ...
  function buildAccordionItem(node, index, depth = 0, rootHeaderId = null){
    ...
    if (showPrivilegeTable){
      if(!hasOwnPrivileges){
        tbody.innerHTML = `
          <tr><td colspan="2">
            <div class="ap-small-muted p-3 text-center">
              <i class="fa fa-ban me-2"></i>No privileges for this module
            </div>
          </td></tr>`;
      } else {
  ...
  async function loadModulesWithPrivileges(){
    ...
    try{
      ...
      console.error(e);
      modulesEmpty.innerHTML = `
        <i class="fa fa-exclamation-triangle text-danger"></i>
        <div class="text-danger">Failed to load modules: ${escapeHtml(e.message || '')}</div>
      `;
      modulesEmpty.style.display = '';
      chkGlobalSelectAll.checked = false;
      chkGlobalSelectAll.indeterminate = false;
  ...
  (async ()=>{
    try{
      ...
      modulesEmpty.innerHTML = `
        <i class="fa fa-exclamation-triangle text-danger"></i>
        <div class="text-danger">Cannot load privileges: ${escapeHtml(e.message || '')}</div>
      `;
      modulesEmpty.style.display = '';
      chkGlobalSelectAll.checked = false;
      chkGlobalSelectAll.indeterminate = false;
